...
- **横贯块左右间距规范**：面板体内 `banner` 等横贯条文字与外缘保持 16px 内缩、横线仍铺满宽度（负 margin 抵消 body padding）；内联/工具卡（`[data-genui]`/`[data-genui-tool]` 无容器 padding）内的 `banner`/`steps` 补左右 16px 防贴边；块组件（callout/card/list）自带形态不动。
