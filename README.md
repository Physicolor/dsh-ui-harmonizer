...
- **Real-DOM 复核结论（诊断为最终版）**：面板实际宽 1049 ≈ 对话列 1047，本身未越界；「视觉探出」真因是 block 元素停在 flex 容器（composerStack）中，其自带 `margin 14px × 2` 不折叠 → 占位 1077 > 列宽 1047，右缘探出 ~30px，且外层 scrollBody `overflow-x:hidden` 使 `scrollWidth > clientWidth` 为 false（无滚动条掩盖了探出，此前误判已修复）。修复 = `[data-genui-panel]{ width:auto; max-width:100% !important; margin:10px auto 2px !important; contain:inline-size }`，水平 margin 转 auto 令占位等于列宽、探出归零。
- **最终基准（用户验收标准 · 定稿）**：面板/工具卡片的宽度基准 = **对话内容宽度**，即 harmonizer「对话列最大宽度」滑杆的 `--enhancer-content-width`（当前 840px），与官方 `Md3f7G_column`（对话内容列）一致，而非输入框或对话列外层。最终规则：`[data-genui-panel]{ display:block; width:100% !important; max-width: var(--enhancer-content-width, 748px) !important; margin:10px auto 2px !important; contain:inline-size; box-sizing:border-box }`——宽度随滑杆即时预览自适应（调滑杆面板立刻跟随），水平居中于对话列。废弃了此前「实测输入框宽写 `--enhc-message-maxw`」的方案（基准偏差）。教训：`width:auto` + `margin:auto` 在 flex 容器交叉轴会触发 shrink-to-fit 把面板压成竖线（回归），必须显式 `width:100%` 再叠加 max-width。
- **横贯块左右间距规范**：`banner` 与折叠条复用同一宽度格式——保持内容宽、左右 16px 内缩（文本起点与 panelToggle 标题一致），不向外负 margin 扩盒（避免 padding 溢出到卡片外围），任意容器（面板体/内联/工具卡）下均成立；`steps` 在无容器 padding 的内联/工具卡内补 16px 左右；块组件（callout/card/list）自带形态不动。
