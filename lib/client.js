		...
		function CreateTaskForm({ onCreated }) {
			...
			const create = async () => {
				...
				try {
					...
					if (scheduleMode === "interval") frequencyMinutes = Math.max(1, Number(intervalValue) || 60);
					else if (scheduleMode === "periodic") frequencyMinutes = 1440;
					await api({
						kind: "tasks/create",
						task: {
							name: name.trim(),
							...
							frequencyMinutes,
							scheduleMode,
							periodicDay: scheduleMode === "periodic" ? Number(periodicDay) : void 0,
							periodicTime: scheduleMode === "periodic" ? periodicTime : void 0
						}
					});
					onCreated();
				} catch (e) {
			...
			return react.createElement("div", {
				...
				}, "添加自动化任务"), react.createElement("div", {
					key: "actions",
					style: {
						display: "flex",
						gap: 8
